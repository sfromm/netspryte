...
class DataWorker(multiprocessing.Process):
    ...
    def process_module_data(self, data_mod):
        ...
        for data in data_mod.data:
            ...
            if not this_host:
                ...
                self.process_host_data(data_mod, this_host)
            if not this_class:
    ...
    def process_measurement_instance_data(self, data, measurement_instance, model):
        ...
        # This is probably supposed to be handled by process_host_data()
        ...
        for field in cls._meta.sorted_fields:
            if field.name == 'id' or isinstance(field, peewee.ForeignKeyField) or field.name not in data:
                continue
            if not field.null:
                init_data[field.name] = data[field.name]
        init_data['measurement_instance'] = measurement_instance
        m = self.mgr.get_or_create(cls, **init_data)
        if m is None:
    ...
    def process_host_data(self, data, host):
        '''
